...
def download(ckpt_name: str) -> str:
    ...
    try:
        ...
        if os.path.exists(
            os.path.join(os.getcwd(), "pretrained", "UniRig", MAP[ckpt_name])
        ):
            print(
                f"Found the checkpoint at {os.path.join(os.getcwd(), 'pretrained', 'UniRig', MAP[ckpt_name])}"
            )
            return os.path.join(os.getcwd(), "pretrained", "UniRig", MAP[ckpt_name])
        return hf_hub_download(repo_id="VAST-AI/UniRig", filename=MAP[ckpt_name])
