...
def load(filepath: str):
    ...
    try:
        if filepath.endswith(".vrm"):
            ...
            bpy.ops.import_scene.fbx(
                filepath=filepath, ignore_leaf_bones=False, use_image_search=False
            )
    ...
    armature = [
        x for x in set(bpy.context.scene.objects) - old_objs if x.type == "ARMATURE"
    ]
    if len(armature) == 0:
        return None
    if len(armature) > 1:
        raise ValueError("multiple armatures found")
    armature = armature[0]
        ...
        try:
            for item in data_collection:
                try:
                    ...
                    print(
                        f"Warning: Could not remove {item.name} from {data_collection}: {e}"
                    )
        except Exception as e:
...
def process_armature(
    ...
    for id, pbone in enumerate(arranged_bones):
        ...
        parents.append(
            None
            if pbone.parent not in arranged_bones
            else name_to_id[pbone.parent.name]
        )
        # remove scale part
...
def save_raw_data(
    ...
    if faces.shape[0] > target_count:
        vertices, faces = fast_simplification.simplify(
            vertices, faces, target_count=target_count
        )
    mesh = trimesh.Trimesh(vertices=vertices, faces=faces)
    ...
    for file in tqdm(files[start:]):
        ...
        try:
            ...
            if armature is not None:
                ...
                joints, tails, parents, names, matrix_local = process_armature(
                    armature, arranged_bones
                )
...
def parse():
    ...
    parser.add_argument(
        "--input_dir", type=nullable_string, required=False, default=None
    )
    parser.add_argument(
        "--output_dir", type=nullable_string, required=False, default=None
    )
...
if __name__ == "__main__":
    ...
    assert config.input_dataset_dir is not None or args.input is None, (
        "you cannot specify both input and input_dir"
    )
